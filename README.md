...
- **一个模型干两份活**：只需一个支持音频输入的多模态模型（如 Qwen3-Omni、GPT-4o-audio），既能直接听视频写总结，也能处理字幕文本
- **画面理解（v0.8.0 新增）**：速成课/教程类视频可开启「AI 看画面」——自动抽取关键帧，逐字转写屏幕上的题目、选项、公式与板书
...
**画面理解（可选，v0.8.0+）**：设置页勾选「画面理解」后，无字幕视频的总结会额外抽取关键帧画面给多模态模型看——速成课、教程类视频里屏幕上出现的题目、选项、公式、板书会被逐字转写进笔记。需模型同时支持图片输入（Qwen3-Omni 支持）；开启后更耗 token。
