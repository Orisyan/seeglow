...
STYLE_GUIDES = {
    ...
    "speed": (
        "【风格：速学模式】目标：让读者 3 分钟吃透视频精华，像过知识卡片一样高效。\n"
        "- 「## 一句话总结」保持\n"
        "- 「## 核心要点」改为「## 必会知识点」：5~8 条，每条以★标注重要度（★★★必会 / ★★重要 / ★了解），"
        "关键数据、术语、公式加粗\n"
        "- 「## 内容时间线」只列干货段落，跳过寒暄、重复与铺垫，格式 `[mm:ss] 主题 —— 一句话结论`\n"
        "- 额外增加「## 自测五题」：5 道简答题覆盖核心考点；每题下一行用引用格式（> ）给出答案，方便先自测后对照\n"
        "- 末尾一行「## 收尾建议」：给一句怎么运用/延伸学习的建议"
    ),
}
